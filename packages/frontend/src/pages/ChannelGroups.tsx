import { useChannelGroups, useChannelGroupMembers, useCreateChannelGroup, useAddChannelGroupMember, useRemoveChannelGroupMember } from '@/hooks/use-groups';
...
export default function ChannelGroups() {
  ...
  const [selectedGroup, setSelectedGroup] = useState<number | null>(null); const [selectedChannel, setSelectedChannel] = useState<number | null>(null); const [newName, setNewName] = useState(''); const [showAddMembers, setShowAddMembers] = useState(false); const [selectedMembers, setSelectedMembers] = useState<number[]>([]);
  const createGroup = useCreateChannelGroup(); const addMember = useAddChannelGroupMember(); const removeMember = useRemoveChannelGroupMember();
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between"><h1 className="text-xl font-semibold">{t('channelGroups.title')}</h1><div className="flex gap-2"><Input className="h-8 w-40" value={newName} onChange={(e) => setNewName(e.target.value)} placeholder={t('channelGroups.newName')} /><Button size="sm" disabled={!newName.trim()} onClick={() => createGroup.mutate(newName, { onSuccess: () => setNewName('') })}><Plus className="h-3 w-3 mr-1" />{t('channelGroups.create')}</Button></div></div>
