...
export function MusicSourcesTab() {
  ...
  const [testResult, setTestResult] = useState<Record<string, { initialized: boolean; keyword: string; requests: { ok: boolean }[]; platforms?: { id: string; name: string; ok: boolean; searchOk: boolean; playOk: boolean; message?: string }[] }>>({});
        ...
        <CardContent className="space-y-4">
          ...
          {isLoading ? <p className="text-sm text-muted-foreground">{t('music.sources.loading')}</p> : sources.length === 0 ? <div className="rounded-lg border border-border p-8 text-center"><FileCode2 className="mx-auto mb-2 h-7 w-7 text-muted-foreground" /><p className="text-sm font-medium">{t('music.sources.empty')}</p><p className="mt-1 text-xs text-muted-foreground">{t('music.sources.emptyHelp')}</p></div> : sources.map((source, index) => (
            <div key={source.id} className="rounded-lg border border-border bg-background/40 p-4">
              ...
              {testResult[source.id] && <div className="mt-3 space-y-1 text-xs"><p className={testResult[source.id].requests.some((request) => request.ok) ? 'text-emerald-600' : 'text-destructive'}>{t('music.sources.testKeyword', { keyword: testResult[source.id].keyword })}</p>{testResult[source.id].platforms?.map((platform) => <p key={platform.id} className={platform.ok ? 'text-emerald-600' : 'text-destructive'}>{platform.ok ? '✓' : '✗'} {platform.name}：{platform.ok ? '可搜索、可播放' : platform.message || '测试失败'}</p>)}</div>}
            </div>
          ))}
        </CardContent>
      </Card>
      <ConfirmDialog open={!!deleteTarget} onOpenChange={(open) => { if (!open) setDeleteTarget(null); }} title={t('music.sources.deleteTitle')} description={t('music.sources.deleteDescription', { name: deleteTarget?.name || t('music.sources.thisSource') })} destructive onConfirm={() => { if (deleteTarget) remove.mutate(deleteTarget.id); }} />
    </div>
  );
}
